...
def install(command: list[str], env: dict[str, str], *, dry_run: bool = False) -> dict[str, Any]:
    path = config_path()
    if dry_run:
        return {"ok": True, "client": "Claude Code", "path": str(path), "detail": ".claude.json"}
    data = read_json(path)
    ...
    return {"ok": True, "client": "Claude Code", "path": str(path), "detail": "mcpServers"}


def uninstall(command: list[str], *, dry_run: bool = False) -> dict[str, Any]:
    path = config_path()
    if not path.is_file():
        return {"ok": False, "client": "Claude Code", "path": str(path), "detail": "missing"}
    if dry_run:
        return {"ok": True, "client": "Claude Code", "path": str(path), "detail": "remove"}
    data = read_json(path)
    servers = data.get("mcpServers")
    if not isinstance(servers, dict) or PRODUCT_NAME not in servers:
        return {"ok": False, "client": "Claude Code", "path": str(path), "detail": "no entry"}
    ...
    write_json(path, data)
    return {"ok": True, "client": "Claude Code", "path": str(path), "detail": "removed"}
