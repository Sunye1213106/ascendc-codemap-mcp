...
def test_query_name_card_from_fixture(tmp_path: Path) -> None:
    ...
    typed = symbol(codemap_id="toy_op@arch35", symbol="IsPse")
    assert typed.get("ok") is True
    assert typed.get("data", {}).get("shape") in {"name", "index"}
...
def test_evidence_id_roundtrip(tmp_path: Path) -> None:
    ...
    op.mkdir()
    _write_fixture(op)
    card = query_codemap(project=str(op), architecture="arch35", pattern="IsPse")
...
def test_query_requires_identity(monkeypatch) -> None:
    ...
    payload = query_codemap(pattern="IsPse")
    assert payload.get("ok") is False
